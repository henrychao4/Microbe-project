...
ind = sample(2, nrow(C_data), replace = TRUE, prob = c(0.7, 0.3))
train = C_data[ind==1,]
test = C_data[ind==2,]

C_rf = randomForest(num_alpha~C_dists, data=train, proximity=TRUE)
print(C_rf)

t_data = data.frame(trait_dists = as.vector(trait_dists), num_alpha = as.vector(num_alpha))

ind = sample(2, nrow(C_data), replace = TRUE, prob = c(0.7, 0.3))
train = t_data[ind==1,]
test = t_data[ind==2,]

t_rf = randomForest(num_alpha~trait_dists, data=train, proximity=TRUE)
print(t_rf)