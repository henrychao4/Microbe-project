...
for (i in 1:n_bootstrap) {
  ...
  clusgap = clusGap(null_I, FUN = wKModes, K.max = k_max, B = 10, weights = eql_abuns, verbose = F)
  boot_max_gaps[i] = max(clusgap$Tab[,3])
}

# for (i in 1:n_bootstrap) {
#   clusgap = clusGap(I, FUN = wKModes, K.max = k_max, B = 10, weights = sample(eql_abuns, length(eql_abuns), replace = F), verbose = F)
#   boot_max_gaps[i] = max(clusgap$Tab[,3])
# }
