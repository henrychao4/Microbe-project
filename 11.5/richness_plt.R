...
sim = ode(y = init_state, times = seq(0, 10000, by = .01), func = MacArthur, parms = params, method = "rk4")
sim.df = as.data.frame(sim)
spec.abuns = sim.df[-((nspec+2):(nspec + nres + 2))]
abuns.df = melt(spec.abuns, id.vars='time')
eql = tail(sim, 1)[-1]
eql_abuns = eql[0:nspec]
richness = rowSums(spec.abuns[-1] > 1)

plot(spec.abuns$time, richness, type = 'l', xlab = 'Time', ylab = 'Richness', col = 'black')
...
kmg_gap = KmeansGap(dat = kmg_data, multiD = T, mink = 1, maxk = 20)
